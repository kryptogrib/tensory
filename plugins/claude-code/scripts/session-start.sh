...
echo "$INPUT" | uvx --refresh-package tensory --from "tensory[mcp,claude-code]" tensory-hook recall 2> >(grep -v "^[[:space:]]*\(Downloading\|Downloaded\|Installed\|Resolved\|Audited\|Prepared\|Using\)" >&2)
